...
public class SlideToRevealImageView: UIView {
    ...
    open func setThumbAtBottom(thumbAtBottom: Bool) {
        _thumbAtBottom = thumbAtBottom
    }
    ...
    open func initialize() {
        ...
        for i in 0 ..< imageViews.count {
            ...
            if (i == 0) {
                bottomAnchor = thumbWrapper.bottomAnchor.constraint(equalTo: imageWrapper.bottomAnchor, constant: 0)
            }
            else {
                bottomAnchor = thumbWrapper.bottomAnchor.constraint(equalTo: thumbWrappers[i-1].topAnchor, constant: 0)
            }
            ...
            if(_thumbAtBottom) {
                NSLayoutConstraint.activate([
                    bottomAnchor,
                    thumbWrapper.heightAnchor.constraint(greaterThanOrEqualToConstant: 0),
                    thumbWrapper.trailingAnchor.constraint(equalTo: imageWrapper.trailingAnchor, constant: CGFloat(thumb.image!.size.width)/2.0),
                    thumbWrapper.widthAnchor.constraint(greaterThanOrEqualToConstant: 0)
                ])
            } else {
                NSLayoutConstraint.activate([
                    topAnchor,
